...
function PlayerArea({ cards, isActive, onCardSelect, onCardAttack, onCardHeal, attackMode, playerType }) {
  ...
  // Cards entrance animation - Fix selector to target player-area
  useGSAP(() => {
  ...
  // Card hover and state animations
  useEffect(() => {
    cardsRef.current.forEach((card, index) => {
      ...
      const ctx = gsap.context(() => {
        card.addEventListener('mouseenter', () => {
          ...
          const isCanAct = cards[index] >= 4;
          const isActionCard = attackMode?.cardIndex === index;
          const isAttackTarget = attackMode?.action === 'attack' && attackMode.player !== playerType;
          const isHealTarget = attackMode?.action === 'heal' && attackMode.player === playerType;

          // Only animate if card is in a special state
          if (isCanAct || isActionCard || isAttackTarget || isHealTarget) {
            gsap.to(card, {
              scale: 1.05,
              y: -4,
              duration: 0.2,
            ...
            // Badge animation only for special states
            ...
            // Minimal animation for knocked out cards
            gsap.to(card, {
              scale: 1.01,
              y: -1,
              duration: 0.2,
        ...
        card.addEventListener('mouseleave', () => {
          // Reset animation only if card was in a special state
          if (cards[index] >= 4 || attackMode || cards[index] === -1) {
            gsap.to(card, {
              scale: 1,
              y: 0,
              boxShadow: 'none',
  ...
  }, [cards, attackMode, playerType]);

  // Active state animation with timeline
  useEffect(() => {
    if (isActive) {
